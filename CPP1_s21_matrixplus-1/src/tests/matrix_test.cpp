...
TEST(Constructor_basic, test_1) {
  ...
  ASSERT_EQ(a.GetRows(), 1);  // assert - утверждения, порождающие в случае их
                              // ложности критические отказы
}
...
TEST(Constructor_basic, test_2) {
  ...
  ASSERT_ANY_THROW(a(1, 2));  // любое исключение
}
...
TEST(Constructor_param, test_7) { ASSERT_ANY_THROW(S21Matrix M(0, 0)); }
...
TEST(Method_Determinant, test_1) {
  ...
  ASSERT_NEAR(
      res, -46.0,
      EPS);  // разница между val1 и val2 не превышает погрешность abs_error
}
...
TEST(Method_Determinant, test_5) {
  ...
  M(3, 3) = 678;
  double res = M.Determinant();
  ASSERT_NEAR(res, -26770223.207819, EPS);
}
...
TEST(Method_SetRows, test_1) {
  S21Matrix M(2, 3);
  ASSERT_ANY_THROW(M.SetRows(0));
}

TEST(Method_SetRows, test_2) {
  S21Matrix M(2, 3);
  ASSERT_ANY_THROW(M.SetRows(-4));
}

TEST(Method_SetCols, test_1) {
  S21Matrix M(2, 3);
  ASSERT_ANY_THROW(M.SetCols(0));
}

TEST(Method_SetCols, test_2) {
  S21Matrix M(2, 3);
  ASSERT_ANY_THROW(M.SetCols(-4));
}
